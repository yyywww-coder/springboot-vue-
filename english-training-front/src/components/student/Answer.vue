...
export default {
  ...
  methods: {
    ...
    getExamData() { //获取当前试卷所有信息
      ...
      this.$axios.get(`/front/exam/examCode?examCode=`+examCode).then(res => {  //通过examCode请求试卷详细信息
        console.log(res);
        this.examData = { ...res.data.data} //获取考试详情
        this.index = 0
        this.time = this.examData.totalScore //获取分钟数
        let paperId = this.examData.paperId
        this.$axios.get(`/front/paper/paperId?paperId=`+paperId).then(res => {  //通过paperId获取试题题目信息
          console.log(res);
